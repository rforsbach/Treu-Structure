...
{
    public abstract class JointRenderer : ItemRenderer
    {
        private Vector3[] symVertices;
        private int[][][] symIndices;
        private int[][][] symbolIndices;

        public Vector3[] SymVertices
        {
        ...
        protected JointRenderer()
        {
            ...
            symVertices = new Vector3[(triangleVertices +           // Triangle vertices... LineList instead of LineStrip for sending bunches of vertices  
            ...
            #region Triangle
            symVertices[vertices++] = new Vector3(-triangleSideSize / 2.0f, -triangleSideSize * 0.6f, 0.0f);
            ...
            #region BaseLine
            symVertices[vertices++] = new Vector3(-baseLineSize / 2.0f, -triangleSideSize * 0.6f, 0.0f);
            symVertices[vertices++] = new Vector3(baseLineSize / 2.0f, -triangleSideSize * 0.6f, 0.0f);
            ...
            symVertices[vertices++] = new Vector3(0.0f, -triangleSideSize * 0.6f, 0.0f);
            symVertices[vertices++] = new Vector3(0.0f, 0.0f, 0.0f);
            ...
            {
                symVertices[vertices++] = new Vector3(-baseLineSize / 2.0f + (baseLineSize / groundLines) * (i / 2.0f + 1.0f), -triangleSideSize * 0.6f, 0.0f);
                symVertices[vertices++] = new Vector3(-baseLineSize / 2.0f + (baseLineSize / groundLines) * (i / 2.0f), -triangleSideSize * 0.7f, 0.0f);
            ...
            {
                symVertices[vertices++] = new Vector3(-baseLineSize / 2.0f + baseLineSize / (2.0f * circlesInSymbol) + circleRadius * (float)Math.Cos(angleStep * (i / 2)),
                                                      -triangleSideSize * 0.6f - circleRadius + circleRadius * (float)Math.Sin(angleStep * (i / 2)), 0.0f);

                symVertices[vertices++] = new Vector3(-baseLineSize / 2.0f + baseLineSize / (2.0f * circlesInSymbol) + circleRadius * (float)Math.Cos(angleStep * (i / 2 + 1)),
                                                      -triangleSideSize * 0.6f - circleRadius + circleRadius * (float)Math.Sin(angleStep * (i / 2 + 1)), 0.0f);
            ...
            for (int i = 1; i < circlesInSymbol; ++i)
            {
                ...
                Vector3 centerOffset = new Vector3(i * baseLineSize / circlesInSymbol, 0.0f, 0.0f);
                for (int j = 0; j < circleSegments * 2; ++j)
                {
                    symVertices[vertices++] = centerOffset + symVertices[baseIndex + j];
            ...
            {
                symVertices[vertices++] = new Vector3(baseLineSize * (-0.5f + 0.05f * (i + 1)), triangleSideSize * (-0.6f - 0.05f * (i + 1)), 0.0f);
            ...
            for (int i = 0; i < translationSpringSegments; ++i)
            {
                ...
                symVertices[vertices++] = new Vector3(x, y, 0.0f);
            ...
            {
                symVertices[vertices++] = new Vector3(x, y, z);
            ...
            for (int k = 1; k < numOptions; ++k)
            {
                ...
                {
                    if( k == 1)
                        symVertices[vertices++] = new Vector3(symVertices[j].X, symVertices[j].Z, symVertices[j].Y);
                    else
                        symVertices[vertices++] = new Vector3(symVertices[j].Z, symVertices[j].X, symVertices[j].Y);
            ...
            for (int k = 1; k < numOptions; ++k)
            {
                ...
                for (int j = symbolBunch; j < bunchSize; ++j)
                {
                    if( k == 1)
                        ...
                        symVertices[vertices++] = new Vector3(symVertices[j].Y, symVertices[j].Z, symVertices[j].X);
        ...
        public int[][] getConstraints(Model.Joint joint, ref bool[] orientation)
        {
            int[][] symVertices;
            bool[] symPossibilities;
            Canguro.Model.JointDOF currDOF;
            ...
            currDOF = joint.DoF;
            byte R = currDOF.AllRestraints;
            byte S = currDOF.AllSprings;
        ...
        public abstract void Render(Device device, Model.Model model, Canguro.Model.Constraint constraint, RenderOptions options);
        public abstract void Render(Device device, Model.Model model, IEnumerable<Model.Joint> joints, RenderOptions options, bool dontDrawInvisibles, bool drawHidden);
    }
}
